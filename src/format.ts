...
// ── Sport Label Caching ──
let lastSportsRef = state.sports;
...
export function formatSportLabel(sport: string): string {
  ...
  // Invalidate cache if sports array reference changes
  ...
  if (sportLabelCache.has(key)) {
    return sportLabelCache.get(key)!;
  }
  ...
  sportLabelCache.set(key, result);
  return result;
}
